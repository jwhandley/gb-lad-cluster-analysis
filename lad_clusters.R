...
parse_sheet <- function(file_path, sheet_name) {
  # Read the sheet
  sheet_data <- suppressMessages(read_excel(file_path, sheet = sheet_name, col_names = FALSE))
  ...
  data_frame <- suppressMessages(read_excel(file_path, sheet = sheet_name, skip = start_row - 1))
  
  if (!any(c("Local Authority District", "County or Unitary Authority") %in% names(data_frame))) {
    return(NULL)
  }
  
  data_frame <- filter(data_frame, !all(is.na(`Local Authority District`),is.na(`County or Unitary Authority`)))
...
read_excel_sheets_auto <- function(file_path) {
  ...
  data_frames <- lapply(sheet_names, function(sheet_name) parse_sheet(file_path, sheet_name))
...
merged_data <- Reduce(function(df1, df2) {
  ...
  duplicated_columns <- setdiff(intersect(names(df1), names(df2)), "Area Code")
...
merged_data %>%
  ...
  select(-c("Lower 95% Confidence Interval","Upper 95% Confidence Interval","Data accuracy","ITL Level 1")) %>%
  mutate(across(c(everything(),-c("Area Code","Local Authority District")),~as.numeric(.x)))-> clean_data
...
ggsave("outputs/lad_clusters.png",width=10,height=16)
